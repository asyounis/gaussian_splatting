...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        view.world_view_transform = torch.nn.Parameter(view.world_view_transform, requires_grad=True)
        view.full_proj_transform = torch.nn.Parameter(view.full_proj_transform, requires_grad=True)
        view.camera_center = torch.nn.Parameter(view.camera_center, requires_grad=True)
        ...
        for i in range(50):
            ...
            loss = torch.mean(torch.abs((rendering - gt.cuda())))

            loss.backward()


            print(view.world_view_transform.grad)
            print(view.full_proj_transform.grad)
            print(view.camera_center.grad)
